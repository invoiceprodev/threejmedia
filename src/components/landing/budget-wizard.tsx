...
export function BudgetWizard({ open, onClose }: BudgetWizardProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => !v && handleClose()}>
      <DialogContent
        mobileFullScreen
        className="gap-0 overflow-hidden bg-gray-950 sm:max-h-[min(90dvh,800px)] sm:max-w-2xl sm:rounded-2xl sm:border-white/10 flex min-h-0 flex-col [&>button]:hidden">
